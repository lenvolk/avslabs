...
$applianceSubscriptionId = 'xxxxxx-xxxxx-xxxxx-xxxxx-xxxxx'
$applianceResourceGroupName = 'volk071-AzMigrate-ARC'
$applianceName = 'Volk071AVSARC'

$customLocationSubscriptionId = 'xxxxxx-xxxxx-xxxxx-xxxxx-xxxxx'
$customLocationResourceGroupName = 'volk071-AzMigrate-ARC'
$customLocationName = 'Volk071NewYork'

$vCenterSubscriptionId = 'xxxxxx-xxxxx-xxxxx-xxxxx-xxxxx'
$vCenterResourceGroupName = 'volk071-AzMigrate-ARC'
$vCenterName = 'Volk071vCenter'
...
try {
    ...
    if ($invokeApplianceRun) {
        ...
        az arcappliance run vmware --tags 'Datacenter=DC01' 'City=NY' @forceParam --subscription $applianceSubscriptionId --resource-group $applianceResourceGroupName --name $applianceName --location $location --address $vcenterDetails[$vcAddressKey] --username $vcenterDetails[$vcUsernameKey] --password $vcenterDetails[$vcPasswordKey]
    }
    else {
    ...
    az customlocation create --debug --tags 'Datacenter=DC01' 'City=NY' --subscription $customLocationSubscriptionId --resource-group $customLocationResourceGroupName --name $customLocationName --location $location --namespace $customLocationNamespace --host-resource-id $applianceId --cluster-extension-ids $ceVmware 2>> $logFile
    ...
    if ($enableAKS -eq 'true') {
        ...
        az customlocation create --debug --tags 'Datacenter=DC01' 'City=NY' --subscription $customLocationSubscriptionId --resource-group $customLocationResourceGroupName --name $customLocationAksName --location $location --namespace $customLocationAksNamespace --host-resource-id $applianceId --cluster-extension-ids $ceAks 2>> $logFile
    ...
    az connectedvmware vcenter connect --tags 'Datacenter=DC01' 'City=NY' --subscription $vCenterSubscriptionId --resource-group $vCenterResourceGroupName --name $vCenterName --custom-location $customLocationId --location $location --fqdn $vcenterDetails[$vcFqdnKey] --port $vcenterDetails[$vcPortKey] --username $vcenterDetails[$vcUsernameKey] --password $vcenterDetails[$vcPasswordKey]
